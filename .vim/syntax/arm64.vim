...
"syn region→ AArch64Included→  display contained start=+"+ skip=+\\\\\|\\"+ end=+"+
"syn match→  AArch64Included→  display contained "<[^>]*>"
"syn match→  AArch64Include→   display "^\s*\zs\(%:\|#\)\s*include\>\s*["<]" contains=cIncluded

syntax case ignore

syntax region AArch64Comment start="//" end="$" keepend contains=AArch64Special,@Spell
syntax region AArch64Comment start="/\*" end="\*/" contains=AArch64Special,@Spell
" MachO uses ; as a comment leader
syntax region AArch64Comment start=";" end="$" contains=todo,@Spell
...
"highlight default link AArch64Include    Include
