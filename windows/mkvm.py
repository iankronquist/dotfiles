...
VHD_SKUS = [
	'vhd_vb_client_enterprise_en-us_vl_lab',
	'vhd_client_enterprise_en-us_vl',
]
WINBUILDS_PREFIX = R'\\winbuilds\release'
dest=os.path.expanduser(R'~\vhds')
UNATTEND_FILE=os.path.expanduser(R'~\VHDs\unattend-x64.xml')
...
def get_flavor():
	...
	build_type = 'fre'
	print('WARNING: FORCING FREE BUILD')
...
def get_lkg():
	branch_regex = r'.* origin/build/(\w+/[\d\.-]+)'
...
def copy_to_vhds(path):
	flavor = get_flavor()
	for sku in VHD_SKUS:
		vhd_dir = os.path.join(path, flavor, 'vhd', sku)
		print(vhd_dir)
		...
		raise Exception(':( Could not find any of these vhd skus: ' + ', '.join(VHD_SKUS))
	print('assdf')
	vhd = os.path.join(vhd_dir, vhds[0])
	print(vhd)
	dest_vhd = os.path.join(dest, vhds[0])

	if not os.path.exists(os.path.dirname(dest_vhd)):
	    os.makedirs(os.path.dirname(dest_vhd))

	if not os.path.exists(dest_vhd):
		print('copying...')
		print(vhd, dest_vhd)
		shutil.copy(vhd, dest_vhd)
		print('copied')
