...
class BugCheckInfo(ctypes.Structure):
    ...
    def __str__(self):
        name = BUG_CHECK_CODE_NAMES.get(self.BugCheckCode)
        if not name:
            name = 'Unknown to this script'
...
def parse_dump_file(dump_name):
    ...
    with open(dump_name, 'rb') as f:
        ...
        if magic != DUMP64_MAGIC and magic[:len(MEMORY_DUMP_MAGIC)] != MEMORY_DUMP_MAGIC:
